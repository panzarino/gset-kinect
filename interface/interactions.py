...
if 1400 > screensize[0] >= 1200:
    fontsize = [125, 50, 30]
    spacesize = [100, 314]
elif 1200 > screensize[0] >= 800:
    fontsize = [100, 75, 30]
    spacesize = [100, 274]
COLORS = {
    'red': (255, 128, 128),
...
def center_text(screen, text_surface, base_y):
    ...
    screen.blit(
        text_surface,
        (screensize[0] / 2 - int(width / 2), base_y)
    )
    return screen
    
CORNERS = {
    'up_left': pygame.Rect((300, 300), (300, 100)),
    'up_right': pygame.Rect((screensize[0] - 500, 300), (300, 100)),
    'down_left': pygame.Rect((300, screensize[1] - 300), (300, 100)),
    'down_right': pygame.Rect((screensize[0] - 500, screensize[1] - 300), (300, 100)) 
}
    
def button(screen, text_surface, corner):
    rect = CORNERS[corner]
    pygame.draw.rect(screen, (250, 250, 250), rect, 5)
...
class Lonely(object):
    def __init__(self, input):
        self.input = input
        self.hello = generate_text("Hello? My name is Charlie.", fontsize[0])
        self.lonely = generate_text("(I'm lonely.)", fontsize[1])
        self.plea = generate_text("Won't you play with me?", fontsize[0])
        self.active = True
...
class Hopeful(object):
    def __init__(self, input):
        self.input = input
        self.active = False
        self.hello = generate_text("Hi!", fontsize[0])
        self.plea = generate_text("Do you want to play with me?", fontsize[0])
        self.yes = generate_text("Yes", fontsize[0])
        self.no = generate_text("No", fontsize[0])
...
class Why(object):
    def __init__(self, input):
        self.input = input
        self.active = False
        self.why = generate_text("Why?", fontsize[0])
        
        self.busy = generate_text("I'm busy", fontsize[2])
        self.hate = generate_text("I don't like you", fontsize[2])
        self.dont_like_games = generate_text("I don't like games", fontsize[2])
        self.back = generate_text("I didn't mean it", fontsize[2])
        
        self.options = Options(['down_left', 'down_right', 'up_left', 'up_right'], self.input)
        
        self.ok = generate_text("Oh, ok.", fontsize[0])
        self.weird = generate_text("You're weird.", fontsize[0])
...
class Choices(object):
    def __init__(self, input):
        ...
        self.text = generate_text("What do you want to do?", fontsize[0])
        self.joke = generate_text("Tell a joke!", fontsize[2])
        self.pong = generate_text("Play ping pong!", fontsize[2])
...
class Joke(object):
    def __init__(self, input):
        ...
        self.text = generate_text("Ok, you start!", fontsize[0])
        self.smile = [
            generate_text(":)", 300),
            generate_text(":|", 300),
            generate_text(":(", 300)
        ]
        self.cannot = generate_text("I can't hear you...", fontsize[0])
    ...
    def loop(self, screen):
        ...
        if delta == 6000:
            screen = center_text(screen, self.cannot, screensize[1] - 150 - 100)
...
class Scared(object):
    def __init__(self, input, next_interaction):
        ...
        self.cry = generate_text("You're hurting me!", fontsize[0])
        self.face = generate_text(":(", 300)
        self.request = generate_text("Could you back away a bit?", fontsize[0])
    ...
    def loop(self, screen):
        ...
        screen = center_text(screen, self.request, screensize[1] - 150 - 100)
