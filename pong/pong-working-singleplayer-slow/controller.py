...
class Controller(object):
	...
	def set(self):
		self.ball = Ball(self.game.screen, self.color['ball'], (self.game_middle_x, self.game_middle_y), 14, 2, 4)
		self.ball.draw()
		self.ball.x = 12
		self.ball.y = 0
	...
	def go(self):
		...
		for e in events:
			if e.type == KINECTEVENT:
				...
				if e.key == K_ESCAPE:
					self.game.quit()
		self.left.move(self.left.rectlist[1])
		self.right.move(self.right.rectlist[1])
		# move ball
		self.ball.change()
		balloldx = self.ball.x
		balloldy = self.ball.y
		# check where ball has to move to
